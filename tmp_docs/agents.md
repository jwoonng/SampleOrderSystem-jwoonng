...
└──────────────┬──────────────────────┘
    ...
    ▼          ▼              ▼              ▼
┌────────┐ ┌────────┐ ┌──────────────┐ ┌─────────────┐
│  Red   │ │ Green  │ │  Verifying   │ │ Refactoring │
...
- Clean Code 원칙에 따라 **변수 → 메서드 → 클래스** 단위로 점진적으로 리팩터링한다.
- **기능 변경 없이** 코드 품질만 개선한다. 로직 수정은 하지 않는다.
- 각 단계(Step) 완료 후 반드시 전체 테스트를 실행하여 통과를 확인한다.
- `docs/prd.md` 요구사항 범위를 벗어난 기능 추가나 아키텍처 변경은 하지 않는다.

#### 리팩터링 단계

| Step | 대상 | 주요 작업 |
|------|------|-----------|
| 1 | 변수 | 이름 개선, 매직 넘버 → `constexpr` 상수, 불필요한 임시 변수 제거 |
| 2 | 메서드 | 긴 함수 분리, 중복 코드 추출, Guard Clause 적용 |
| 3 | 클래스 | SRP 위반 분리, 접근 제한자 최소화, 인터페이스 분리 |
