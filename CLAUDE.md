...
> **[중요]** 아래 상황에서는 반드시 작업을 멈추고 개발자의 검증을 받은 후 진행한다.
> - `docs/architecture.md` 수정이 필요하다고 판단될 때
> - `docs/prd.md`의 요구사항이 불명확하거나 해석이 분기될 때
> - 구현 중 아키텍처 또는 요구사항과 충돌이 발생할 때
...
- **[중요]** 커밋은 개발자의 명시적 허락을 받은 후에만 실행한다. 구현이 완료되더라도 개발자가 "커밋해줘" 또는 이에 준하는 명령을 내리기 전까지 `git commit`을 실행하지 않는다.
- **[중요]** 푸시(`git push`)도 개발자의 명시적 허락을 받은 후에만 실행한다. 커밋이 완료되더라도 개발자가 "푸시해줘" 또는 이에 준하는 명령을 내리기 전까지 `git push`를 실행하지 않는다.
